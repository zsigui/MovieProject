...
abstract class ParameterHandler<T> {
  ...
  // 自定义的Query类型，此处其实基本不做工作，只是为了提供对应的执行形式
  static final class FixedQuery extends ParameterHandler<String> {
    private final String name;
    private final boolean encoded;

    FixedQuery(String name, boolean encoded) {
      this.name = checkNotNull(name, "name == null");
      this.encoded = encoded;
    }

    @Override void apply(RequestBuilder builder, String value) throws IOException {
      if (value == null) return; // Skip null values.
